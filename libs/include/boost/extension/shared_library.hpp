...
public:
  ...
  /** set new location of shared library
    * if library is opened then it's closed, and new library is loaded.
    * \param location The relative or absolute path of the shared library.
    * \param auto_open If true then library is automatically loaded
    * \return true if the shared library was opened successfully.
    * \pre None.
    * \post If true is returned, the shared library is opened and
    * get() can be called.
    */
  bool set_location(const std::string& location, bool auto_open = false) {
    if (auto_open) {
	  if (handle_ != 0)
	    close();
  ...
  /** Unsafe version of get.
    *
    */
  impl::generic_function_ptr getptr(const std::string& name) {
	  return impl::get_function(handle_, name.c_str());
  }
