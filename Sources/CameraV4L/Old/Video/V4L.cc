...
V4L::V4L(string param, string value) {
	...
	if (param.compare("device") == 0)
		...
		XMLDataSynchronizer* xmlds = XMLDataSynchronizer::getInstance();
		which_dev = xmlds->getAttributeValue(
				"/settings/sources/CameraV4L/input_device", "device");
	}
	string dev_name = "/dev/" + which_dev;
	char * device = (char*) (dev_name.c_str());
...
bool V4L::getDeviceCapabilities() {
	...
	if (ioctl(video_dev, VIDIOCGCAP, &capabilities) == -1) {
		printf("ERROR: function: getDeviceCapabilities\n");
		close(video_dev);
		return false;
	}
	return true;
...
int V4L::getVideoProperty(int property) {
	...
	for (i = 0; i < capabilities.channels; i++) {
		...
		if (ioctl(video_dev, VIDIOCGCHAN, &vidchan) == -1) {
			printf("ERROR: function: getChannelCapabilities\n");
			return 0;
		}
	}
	return 1;
...
bool V4L::setVideoProperty(int number, int norm) {
	...
	if (ioctl(video_dev, VIDIOCSCHAN, &vidchan) == -1) {
		printf("ERROR: function: setChannelCapabilities\n");
		return false;
	} else
		return true;
...
int V4L::getWinProperty(int property) {
	...
	if (property < 2) {
		if (ioctl(video_dev, VIDIOCGWIN, &win) == -1) {
			printf("ERROR: function: getWinProperties\n");
			return 0;
		} else {
			if (property == 0)
				return win.width;
...
bool V4L::setWinProperty(int property, int value) {
	...
	if (property != 2) {
		if (ioctl(video_dev, VIDIOCSWIN, &win) == -1) {
			printf("ERROR: function: setWinProperties\n");
			return false;
		} else
			return true;
	} else {
...
bool V4L::setPicProperty(int property, int value) {
	...
	if (ioctl(video_dev, VIDIOCSPICT, &pic) == -1) {
		printf("ERROR: function: setPicProperties\n");
		return false;
	} else
		return true;
}
...
bool V4L::getBufferSize() {
	...
	if (ioctl(video_dev, VIDIOCGMBUF, &m_buf) == -1) {
		printf("ERROR: function: getBufferSize\n");
		return false;
	}
...
bool V4L::setMemBuf() {
	...
	if ((unsigned char *) -1 == (unsigned char *) video_dev) {
		printf("ERROR: function: setMemBuf\n");
		return false;
	}
	return true;
}
...
bool V4L::retFrame() {
	if (ioctl(video_dev, VIDIOCSYNC, &((*(v_map + bufferIndex)).frame)) == -1) {
		printf("ERROR: function: retFrame\n");
		return false;
	}
...
bool V4L::loadFrameGrabber(int version) {
	...
	if (version > 0) {
		...
		string s_min_settings = xmlds->getAttributeValue(
				"/settings/sources/CameraV4L/input_device", "min_settings");
