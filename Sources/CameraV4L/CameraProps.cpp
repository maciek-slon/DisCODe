...
void CameraProps::load(const ptree & pt) {
	...
	io       = cam->convIOMethod(pt.get("device.io", "MMAP"));
	standard = cam->convStandard(pt.get("device.video_standard", "PAL"));
	width    =                   pt.get("device.width", 640);
	height   =                   pt.get("device.height", 480);
	channel  = cam->convChannel( pt.get("device.channel", "Composite"));
}
