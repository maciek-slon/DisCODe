...
Blob::Blob(t_labelType id, CvPoint startPoint, CvSize originalImageSize)
{
	...
	m_externalContour = BlobContour(startPoint, m_storage);
...
Blob& Blob::operator=(const Blob &src)
{
	if (this != &src) {
		...
		m_externalContour = BlobContour(src.m_externalContour.GetStartPoint(), m_storage);
		...
		if (src.m_internalContours.size()) {
			...
			while (itSrc != src.m_internalContours.end()) {
				*it = BlobContour((*itSrc).GetStartPoint(), m_storage);
				if ((*itSrc).m_contour)
...
/*!
 * \brief Get blob area, ie. external contour area minus internal contours area.
 * \return blob area
 */
double Blob::Area()
{
...
double Blob::ExternPerimeter(IplImage *maskImage, bool xBorder /* = true */, bool yBorder /* = true */)
{
	...
	externalPoints = cvCreateSeq(externContour->flags, externContour->header_size, externContour->elem_size, m_storage);
	...
	for (j = 0; j < externContour->total; j++) {
		...
		if (xBorder & ((actualPoint.x == 0) || (actualPoint.x == m_originalImageSize.width - 1)) || yBorder
				& ((actualPoint.y == 0) || (actualPoint.y == m_originalImageSize.height - 1))) {
			find = true;
		} else {
			if (maskImage != NULL) {
...
BlobContour::t_PointList Blob::GetConvexHull()
{
	...
	if (m_externalContour.GetContourPoints())
		convexHull = cvConvexHull2(m_externalContour.GetContourPoints(), m_storage, CV_COUNTER_CLOCKWISE, 1);
